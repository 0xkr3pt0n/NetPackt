...
def scan_host(ip, port=445):
	try:
		...
		if response_code == "050200c0":
			response_code = 1
			result_respons = "[+] %s, VULNERABLE (STATUS_INSUFF_SERVER_RESOURCES)" % (ip)
			return result_respons, response_code

		elif (response_code == "080000c0") or (response_code == "02005b00"):
			response_code = 0
			result_respons = "[-] %s, NOT VULNERABLE" % (ip)
			return result_respons, response_code

		elif (response_code == "02000500"):
			response_code = 0
			result_respons = "[-] %s, ERROR (TID INVALID)" % (ip)
			return result_respons, response_code
			
		elif (response_code == "220000c0"):
			response_code = 0
			result_respons = "[-] %s, NOT VULNERABLE (STATUS_ACCESS_DENIED)" % (ip)
			return result_respons, response_code
			
		else:
			response_code = 0
			result_respons = "[-] %s, UNABLE TO DETERMINE (%s)" % (ip, response_code)
			return result_respons, response_code

	except Exception as err:
		response_code = -1
		result_respons = "[-] {}, EXCEPTION: {}".format(ip, str(err).upper())
		return result_respons, response_code
